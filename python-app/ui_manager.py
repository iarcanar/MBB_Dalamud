...
class UIManager:
    ...
    def create_toggle_switch(
        ...
        # สร้าง bg switch - CYBERPUNK: Use theme colors
        ...
        def toggle_switch():
            ...
            # อัพเดต UI - CYBERPUNK: Use theme colors
            if variable.get():  # เปิด
                indicator.place(x=x_on)
                switch_bg.config(bg=appearance_manager.get_theme_color("accent", "#00FFFF"))  # Cyan
            else:  # ปิด
                indicator.place(x=x_off)
                switch_bg.config(bg=appearance_manager.get_theme_color("button_inactive_bg", "#2a2a3e"))  # Dark blue-gray
    ...
    def create_cyber_border(self, canvas, x1, y1, x2, y2, width=2, corner_size=10):
        """
        วาดขอบแบบไซเบอร์พังก์พร้อมมุมเน้น

        Args:
            canvas: tk.Canvas ที่จะวาด
            x1, y1: พิกัดมุมบนซ้าย
            x2, y2: พิกัดมุมล่างขวา
            width: ความกว้างของขอบ (default: 2)
            corner_size: ขนาดของเส้นมุม (default: 10)

        Returns:
            list: รายการ ID ของ canvas items ที่สร้าง
        """
        border_color = self.get_theme_color("border") if hasattr(self, "get_theme_color") and "border" in self.get_all_themes().get(self.current_theme, {}) else "#00FFFF"
        items = []
        ...
        # วาดมุมเน้น (รูป L ที่แต่ละมุม)
        accent_width = width + 1

        # มุมบนซ้าย
        items.append(canvas.create_line(
            x1, y1, x1 + corner_size, y1,
            fill=border_color, width=accent_width
        ))
        items.append(canvas.create_line(
